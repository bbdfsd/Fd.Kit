...
namespace Fd.Kit.BasicManagement.AuditLogs
{
    ...
    [DisableAuditing]
    public class AuditLogsAppService : BasicManagementAppService, IAuditLogsAppService
    {
        ...
        readonly IPermissionDefinitionManager PermissionDefinitionManager;
        ...
        {
            var result = await AuditLogRepository.GetAverageExecutionDurationPerDayAsync(filter.StartDate, filter.EndDate);
        ...
        protected virtual async Task CheckPermissionForEntity(string entityFullName)
        {
            ...
            {
                var hasPermission = await PermissionChecker.IsGrantedAsync(permissionName);
                if (!hasPermission)
                {
                    await AuthorizationService.CheckAsync(AbpAuditLoggingPermissions.AuditLogs.Default);
                }
